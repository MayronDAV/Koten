...
namespace KTN
{
    ...
    void Editor::Shortcuts()
    {
        ...
        if (m_State == RuntimeState::Edit && Shortcuts::IsActionPressed("Play"))
        {
            UnSelectEntt();
            SceneManager::Play();
            m_State = RuntimeState::Play;
        }

        if (m_State != RuntimeState::Edit && Shortcuts::IsActionPressed("Stop"))
        {
            SceneManager::Stop();
            m_State = RuntimeState::Edit;
        }

        if (m_State == RuntimeState::Edit && !Input::IsMouseButtonPressed(Mouse::Button_Right))
        {
